...
export class EventCollector {
  ...
  updateIndicators(
    ...
    // Track which conditions need checking for change events
    ...
    for (const [key, newValue] of states) {
      ...
      if (previousValue !== newValue) {
        ...
        // Calculate snapshot AFTER this flip
  ...
  private calculateConditionSnapshot(conditionType: ConditionType): ConditionSnapshot {
    ...
    // Condition is met if:
    // - All required are true
    // - AND (no optionals OR at least one optional is true)
  ...
  private checkConditionChange(
    ...
    if (previousMet !== newMet) {
      // Find triggering indicator (the last one that flipped to cause this change)
      // This is the most recent flip event for this condition
  ...
  buildTradeEvents(): TradeEvent[] {
    ...
    // First pass: collect entry swaps by their position in sequence
    let swapPairIndex = 0;
    ...
    // Second pass: pair with exit swaps
    swapPairIndex = 0;
