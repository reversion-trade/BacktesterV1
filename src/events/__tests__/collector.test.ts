...
describe("updateIndicators", () => {
  ...
  it("detects indicator flip from false to true", () => {
    ...
    const flipEvents = events.algoEvents.filter(
      (e) => e.type === "INDICATOR_FLIP"
  ...
  it("detects indicator flip from true to false", () => {
    ...
    const trueToFalseFlip = flipEvents.find(
      (e) => e.barIndex === 1 && e.indicatorKey === "rsi14"
    );
...
describe("condition change detection", () => {
  ...
  it("identifies triggering indicator key", () => {
    ...
    const events = collector.getEvents();
    const conditionEvent = events.algoEvents.find(
      (e) => e.type === "CONDITION_CHANGE"
    ) as ConditionChangeEvent;
...
describe("emitStateTransition", () => {
  ...
  it("emits state transition event", () => {
    ...
    const events = collector.getEvents();
    const stateEvents = events.algoEvents.filter(
  ...
  it("includes trade ID when in position", () => {
    ...
    const events = collector.getEvents();
    const stateEvent = events.algoEvents.find(
      (e) => e.type === "STATE_TRANSITION"
    ) as StateTransitionEvent;
...
describe("emitSpecialIndicatorEvent", () => {
  ...
  it("emits SL_SET event", () => {
    ...
    const events = collector.getEvents();
    const specialEvents = events.algoEvents.filter(
      (e) => e.type === "SL_SET"
    );
  ...
  it("emits SL_HIT event", () => {
    ...
    const events = collector.getEvents();
    const hitEvents = events.algoEvents.filter(
      (e) => e.type === "SL_HIT"
    );
