...
class MockExecutor implements IExecutor {
  ...
  async placeOrder(order: OrderRequest): Promise<OrderResult> {
    ...
    const amount = order.amountUSD
      ? order.amountUSD / this.currentPrice
      : order.amountAsset ?? 0;
...
class MockIndicatorFeed implements IIndicatorFeed {
  ...
  getConditionSnapshot(conditionType: ConditionType): ConditionSnapshot {
    return this.conditionSnapshots.get(conditionType) ?? {
      requiredTrue: 0,
      requiredTotal: 1,
      optionalTrue: 0,
      optionalTotal: 0,
