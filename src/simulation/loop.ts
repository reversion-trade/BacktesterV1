 ...
 *
 * @deprecated This is the LEGACY simulation engine. Use `AlgoRunner` from
 * `./algo-runner.ts` or `runBacktestPipeline` from `./stages/index.ts` instead.
 *
 * The modern `AlgoRunner` uses dependency injection (IExecutor, IDatabase,
 * IIndicatorFeed) which ensures the same code runs in both backtest and live
 * trading. This file is kept for backward compatibility only.
 *
 * Migration path:
 * - For backtest API: Use `runBacktestWithEvents()` from `src/index.ts`
 * - For direct access: Use `runBacktestPipeline()` from `./stages/index.ts`
 *
 ...
 *
 * @audit-trail
 * - Deprecated: 2026-01-05 (Refactoring: Consolidate to single simulation engine)
 */

import type { Candle, AlgoParams, Direction, PositionState, EntryCondition, ExitCondition } from "../core/types.ts";
 ...
 */
function calculatePositionSize(algoParams: AlgoParams, currentEquity: number): number {
 ...
 *
 * @deprecated Use `runBacktestPipeline()` from `./stages/index.ts` or
 * `runBacktestWithAlgoRunner()` from `./algo-runner.ts` instead.
 *
...
export function runSimulation(config: SimulationConfig): SimulationResult {
    ...
    for (let i = 0; i < candles.length; i++) {
        ...
        if (activePosition !== null && currentState !== "FLAT") {
            ...
            const exitSignalTriggered = assumePositionImmediately ? exitConditionMet : !prevExitMet && exitConditionMet;
        ...
        if (currentState === "FLAT" && !isWarmupPeriod) {
            ...
            if (!hitTradesLimit) {
                ...
                const longEntryTriggered = assumePositionImmediately ? longEntryMet : !prevLongEntryMet && longEntryMet;
...
function enterPosition(
    ...
    if (exitCondition?.stopLoss) {
        ...
        collector.emitSpecialIndicatorEvent(
            ...
            "SL_SET",
            candle.close,
            position.stopLoss.getStopLossPrice(),
            direction
        );
    }
    ...
    if (exitCondition?.takeProfit) {
        ...
        collector.emitSpecialIndicatorEvent(
            ...
            "TP_SET",
            candle.close,
            position.takeProfit.getTakeProfitPrice(),
            direction
        );
    }
    ...
    if (exitCondition?.trailingSL && exitCondition?.stopLoss) {
        ...
        collector.emitSpecialIndicatorEvent(
            ...
            "TRAILING_SET",
            candle.close,
            position.trailingStop.getCurrentLevel?.() ?? 0,
            direction
        );
    }
