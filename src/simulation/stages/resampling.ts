...
export function executeResampling(input: ResamplingInput): ResamplingResult {
  ...
  for (const key of signalCache.keys()) {
    ...
    const signalTimestamps = createSignalTimestamps(
      startTime,
      originalSignals.length,
      indicatorResolution
    );

    // Resample to simulation resolution
    const resampled = resampleSignals(
      originalSignals,
      signalTimestamps,
      simulationTimestamps
    );
  ...
  const warmupBars = Math.ceil(
    (warmupCandles * MIN_SIMULATION_RESOLUTION) / simulationResolution
  );
 ...
 */
export function createResamplingInput(
  candles: Candle[],
  indicatorResult: IndicatorCalculationResult
): ResamplingInput {
  return {
    candles,
    signalCache: indicatorResult.signalCache,
...
export function validateResamplingResult(result: ResamplingResult): {
  ...
  if (result.warmupBars >= result.totalSimulationBars) {
    issues.push(
      `Warmup (${result.warmupBars}) >= total bars (${result.totalSimulationBars})`
    );
  }
 ...
 */
export function getTimestampForBar(
