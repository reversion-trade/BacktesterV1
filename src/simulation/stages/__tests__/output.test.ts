...
function createMockAlgoEvent(
  overrides: Partial<AlgoEvent> = {}
): AlgoEvent {
  return {
    timestamp: 1000,
    barIndex: 0,
...
function createMockEquityPoint(
...
function createMockBacktestOutput(
  overrides: Partial<BacktestOutput> = {}
): BacktestOutput {
  return {
    config: {
      algoId: "test-algo",
...
describe("calculateMetrics", () => {
  ...
  it("calculates swap metrics from trades", () => {
    const trades = [
      createMockTradeEvent({ pnlUSD: 100 }),
      createMockTradeEvent({ pnlUSD: -50 }),
    ];
