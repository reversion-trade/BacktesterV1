...
import type { BalanceResult, IntraTradeExtremes } from "./types.ts";
 ...
 */
export class BalanceIndicator extends BaseSpecialIndicator<
  BalanceConfig,
  BalanceResult
> {
  ...
  calculateRealizedPnL(exitPrice: number): number {
    ...
    if (this.config.direction === "LONG") {
      grossPnL =
        this.positionSizeAsset *
        (effectiveExitPrice - this.effectiveEntryPrice);
    } else {
      grossPnL =
        this.positionSizeAsset *
        (this.effectiveEntryPrice - effectiveExitPrice);
    }
