 ...
 */
export function runBacktestWithEvents(
  candles: Candle[],
  input: BacktestInput
): BacktestOutput {
  ...
  const filteredCandles = candles.filter(
    (c) => c.bucket >= runSettings.startTime! && c.bucket <= runSettings.endTime!
  );

  if (filteredCandles.length === 0) {
    return createEmptyBacktestOutput(validatedInput, startTimeMs);
  }

  // Collect all indicator configs and calculate signals
  const indicatorConfigs = collectIndicatorConfigs(algoConfig.params);
  const { signals: signalCache, warmupCandles } = calculateIndicators(
    filteredCandles,
    indicatorConfigs
  );

  // Run simulation with event collection
  const simConfig: SimulationConfig = {
    candles: filteredCandles,
    signalCache,
    algoParams: algoConfig.params,
    symbol: runSettings.coinSymbol,
    initialCapital: algoConfig.params.startingCapitalUSD * runSettings.capitalScaler,
    feeBps,
    slippageBps,
    warmupCandles,
  ...
  // Process equity curve for storage
  const processedEquityCurve = processEquityCurve(
    simResult.equityCurve.map((p) => ({
      time: p.timestamp,
      equity: p.equity,
      drawdownPct: p.drawdownPct,
      runupPct: 0, // Simplified
    })),
    { targetPoints: 500, preserveDrawdownPeaks: true }
  );

  // Calculate metrics
  const actualStartTime = filteredCandles[0]?.bucket ?? runSettings.startTime!;
  const actualEndTime = filteredCandles[filteredCandles.length - 1]?.bucket ?? runSettings.endTime!;
  const initialCapital = algoConfig.params.startingCapitalUSD * runSettings.capitalScaler;

  const equityCurveForMetrics = simResult.equityCurve.map((p) => ({
    timestamp: p.timestamp,
    equity: p.equity,
    drawdownPct: p.drawdownPct,
  }));

  const swapMetrics = calculateSwapMetrics(simResult.trades, equityCurveForMetrics);
  const algoMetrics = calculateAlgoMetrics(simResult.algoEvents, filteredCandles.length);
  ...
  return {
    config: {
      ...
      startingCapitalUSD: initialCapital,
      feeBps,
      slippageBps,
    },
    events: {
      swapEvents: simResult.swapEvents,
      algoEvents: simResult.algoEvents,
    },
    trades: simResult.trades,
    equityCurve: simResult.equityCurve.map((p) => ({
      timestamp: p.timestamp,
      equity: p.equity,
      drawdownPct: p.drawdownPct,
    })),
    ...
    totalBarsProcessed: filteredCandles.length,
  };
}
...
function runBacktestInternal(
  ...
  // Filter candles to time range
  const filteredCandles = candles.filter(
    (c) => c.bucket >= config.startTime && c.bucket <= config.endTime
  );
 ...
 */
function convertTradeEventsToRecords(
  trades: TradeEvent[],
 ...
 */
function createEmptyBacktestOutput(
  input: BacktestInput,
  startTimeMs: number
): BacktestOutput {
  const endTimeMs = Date.now();
