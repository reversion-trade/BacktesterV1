...
function createRunSettings(algoConfig: AlgoConfig, startTime: number, endTime: number): RunSettings {
    return {
...
describe("EMA Crossover Backtest", () => {
    ...
    test("should run full backtest pipeline with EMA crossover strategy", async () => {
        ...
        console.log(
            `Entry: $${worstTrade.entrySwap.fromAmount.toFixed(2)} → ${worstTrade.entrySwap.toAmount.toFixed(6)} ${worstTrade.entrySwap.toAsset} @ $${worstTrade.entrySwap.price.toFixed(2)}`
        );
        console.log(
            `Exit: ${worstTrade.exitSwap.fromAmount.toFixed(6)} ${worstTrade.exitSwap.fromAsset} → $${worstTrade.exitSwap.toAmount.toFixed(2)} @ $${worstTrade.exitSwap.price.toFixed(2)}`
        );
        ...
        const relevantSwaps = output.events.swapEvents.filter(
            (e) =>
        ...
        for (const swap of relevantSwaps) {
            ...
            console.log(
                `Bar ${swap.barIndex}: ${type} ${swap.fromAmount.toFixed(6)} ${swap.fromAsset} → ${swap.toAmount.toFixed(6)} ${swap.toAsset} @ $${swap.price.toFixed(2)}`
            );
        }
        ...
        for (let i = 0; i < Math.min(20, output.events.swapEvents.length); i++) {
            ...
            console.log(
                `${i + 1}. Bar ${swap.barIndex}: ${entryLabel}(${dirLabel}) ${swap.fromAmount.toFixed(6)} ${swap.fromAsset} → ${swap.toAmount.toFixed(6)} ${swap.toAsset}`
            );
        }
        ...
        for (let i = 0; i < Math.min(5, output.trades.length); i++) {
            ...
            console.log(
                `  Entry: $${trade.entrySwap.fromAmount.toFixed(2)} → ${trade.entrySwap.toAmount.toFixed(6)} ${trade.entrySwap.toAsset}`
            );
            console.log(
                `  Exit: ${trade.exitSwap.fromAmount.toFixed(6)} ${trade.exitSwap.fromAsset} → $${trade.exitSwap.toAmount.toFixed(2)}`
            );
    ...
    test("should handle first week subset", async () => {
        ...
        const algoParams = createEMACrossoverAlgoParams(9, 21, 0.02);
        const algoConfig = createAlgoConfig(algoParams);
        const runSettings = createRunSettings(algoConfig, subset[0]!.bucket, subset[subset.length - 1]!.bucket);
