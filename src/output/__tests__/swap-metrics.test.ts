...
import {
  ...
  getSwapVolumeStats,
} from "../swap-metrics.ts";
import type { SwapEvent, TradeEvent } from "../../events/types.ts";
...
function createEquityCurve(
  equities: number[]
): Array<{ timestamp: number; equity: number; drawdownPct: number }> {
  ...
  describe("Calmar ratio", () => {
    ...
    it("calculates positive Calmar for profitable strategy with drawdowns", () => {
      ...
      const equityCurve = createEquityCurve([
        10000, 10200, 10100, 10400, 10300, 10500
      ]);
...
describe("getSwapVolumeStats", () => {
  ...
  it("uses toAmount for exit swaps (asset to USD)", () => {
    const swaps = [
      createMockSwapEvent({ fromAsset: "BTC", toAsset: "USD", fromAmount: 0.1, toAmount: 4200 }),
    ];
...
describe("Edge Cases", () => {
  ...
  it("handles very small numbers", () => {
    const trades = [
      createTradeEvent("LONG", 0.0001),
      createTradeEvent("LONG", -0.00005),
    ];
    const result = calculateSwapMetrics(trades, []);
  ...
  it("handles very large numbers", () => {
    const trades = [
      createTradeEvent("LONG", 1000000000),
      createTradeEvent("LONG", -500000000),
    ];
    const result = calculateSwapMetrics(trades, []);
