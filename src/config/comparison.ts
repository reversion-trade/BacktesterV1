...
import type { VersionedAlgoConfig, ConfigDiff, VersionComparison, ChangeType } from "./types.ts";
 ...
 */
function deepCompare(pathPrefix: string, oldValue: unknown, newValue: unknown, diffs: ConfigDiff[]): void {
    // Both undefined/null
    ...
    if (Array.isArray(oldValue) && Array.isArray(newValue)) {
        ...
        for (let i = 0; i < maxLen; i++) {
            deepCompare(`${pathPrefix}[${i}]`, oldValue[i], newValue[i], diffs);
        }
        return;
    }
 ...
 */
export function compareAlgoParams(paramsA: AlgoParams, paramsB: AlgoParams): ConfigDiff[] {
 ...
 */
export function compareConfigs(configA: VersionedAlgoConfig, configB: VersionedAlgoConfig): VersionComparison {
...
export function getKeyChanges(comparison: VersionComparison): string[] {
    ...
    for (const diff of comparison.differences) {
        ...
        const isKeyChange = keyPaths.some((kp) => diff.path === kp || diff.path.startsWith(`${kp}.`));
 ...
 */
export function isSuperset(configA: VersionedAlgoConfig, configB: VersionedAlgoConfig): boolean {
 ...
 */
export function isBackwardsCompatible(configA: VersionedAlgoConfig, configB: VersionedAlgoConfig): boolean {
    const comparison = compareConfigs(configA, configB);
