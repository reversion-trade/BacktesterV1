...
import type { AlgoParams, AlgoConfig, RunSettings, EntryCondition, ExitCondition, ValueConfig } from "../core/types.ts";
 ...
 */
function issue(path: string, severity: ValidationSeverity, message: string, code: string): ValidationIssue {
...
function validateValueConfig(
    ...
    if (config.type === "REL" && (config.value < 0 || config.value > 1)) {
        issues.push(
            issue(`${path}.value`, "warning", "REL value should be between 0 and 1 (percentage)", "REL_VALUE_RANGE")
        );
    }
    ...
    if (config.type === "DYN" && !config.valueFactor) {
        issues.push(
            issue(
                `${path}.valueFactor`,
                "warning",
                "DYN type should have valueFactor for modulation",
                "MISSING_VALUE_FACTOR"
            )
        );
    }
}
...
function validateEntryCondition(
    ...
    if (!condition.required || condition.required.length === 0) {
        issues.push(
            issue(`${path}.required`, "error", "At least one required indicator is needed", "NO_REQUIRED_INDICATORS")
        );
    }
    ...
    condition.required?.forEach((ind, idx) => {
        if (!ind.type) {
            issues.push(
                issue(`${path}.required[${idx}].type`, "error", "Indicator type is required", "MISSING_INDICATOR_TYPE")
            );
        }
    });
    ...
    condition.optional?.forEach((ind, idx) => {
        if (!ind.type) {
            issues.push(
                issue(`${path}.optional[${idx}].type`, "error", "Indicator type is required", "MISSING_INDICATOR_TYPE")
            );
        }
    });
}
...
function validateExitCondition(
    ...
    if (!hasSignalIndicators && !hasSLTP) {
        issues.push(
            issue(path, "warning", "Exit has no indicators and no SL/TP - will never exit", "NO_EXIT_MECHANISM")
        );
    }
    ...
    if (condition.trailingSL && !condition.stopLoss) {
        issues.push(
            issue(`${path}.trailingSL`, "error", "Trailing SL requires stopLoss to be set", "TRAILING_WITHOUT_SL")
        );
    }
    ...
    if (needsSymbol && !params.coinSymbol) {
        issues.push(
            issue(
                "coinSymbol",
                "warning",
                "coinSymbol recommended when using ABS stop loss/take profit",
                "MISSING_COIN_SYMBOL"
            )
        );
    }
        ...
        if (settingsSymbol && settingsSymbol !== algoConfig.params.coinSymbol) {
            issues.push(
                issue(
                    "coinSymbol",
                    "warning",
                    "Coin symbol differs from AlgoConfig (will use AlgoConfig value)",
                    "SYMBOL_MISMATCH"
                )
            );
        }
