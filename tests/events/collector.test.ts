...
import { EventCollector, type IndicatorInfo } from "../../src/events/collector.ts";
import type { IndicatorFlipEvent, ConditionChangeEvent, StateTransitionEvent, ConditionType } from "../../src/events/types.ts";
...
describe("emitStateTransition", () => {
    ...
    it("emits state transition event", () => {
        collector.emitStateTransition(0, 1000, "CASH", "POSITION", "ENTRY_SIGNAL");
        ...
        expect(stateEvents[0]!.toState).toBe("POSITION");
    ...
    it("updates current state", () => {
        ...
        collector.emitStateTransition(0, 1000, "CASH", "POSITION", "ENTRY_SIGNAL");
        expect(collector.getCurrentState()).toBe("POSITION");

        collector.emitStateTransition(1, 1001, "POSITION", "CASH", "EXIT_SIGNAL");
        expect(collector.getCurrentState()).toBe("CASH");
    });
