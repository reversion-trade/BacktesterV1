...
} from "../../src/simulation/special-indicators/index.ts";
...
describe("StopLossIndicator", () => {
    describe("LONG Position", () => {
        test("triggers when price falls below stop level (REL)", () => {
            ...
            expect(results.map(r => r.hit)).toEqual([false, false, true]);
        ...
        test("triggers when price equals stop level exactly", () => {
            ...
            expect(results.map(r => r.hit)).toEqual([true]);
        });
        ...
        test("does not trigger when price stays above stop", () => {
            ...
            expect(results.map(r => r.hit)).toEqual([false, false, false]);
        ...
        test("stays triggered once hit", () => {
            ...
            const results = sl.calculate([48000, 50000, 51000], [1001, 1002, 1003]);
            // First price triggers, subsequent stay true even if price recovers
            expect(results.map(r => r.hit)).toEqual([true, true, true]);
        });
    });
...
describe("TrailingStopIndicator", () => {
    describe("LONG Position", () => {
        test("ratchets up with price and triggers on pullback", () => {
            ...
            const results = ts.calculate([51000, 52000, 51500, 50000], [1001, 1002, 1003, 1004]);
        ...
        test("does not trigger when price stays above trailing level", () => {
            ...
            const results = ts.calculate([51000, 52000, 53000], [1001, 1002, 1003]);
    ...
    describe("SHORT Position", () => {
        test("ratchets down with price and triggers on rally", () => {
            ...
            const results = ts.calculate([49000, 48000, 48500, 50000], [1001, 1002, 1003, 1004]);
    ...
    describe("Edge Cases", () => {
        test("stays triggered once hit", () => {
            ...
            const results = ts.calculate([55000, 48000, 60000], [1001, 1002, 1003]);
...
describe("BalanceIndicator", () => {
    ...
    describe("Factory Functions", () => {
        test("createBalanceWithDefaults uses default fees", () => {
            const bal = createBalanceWithDefaults("LONG", 10000, { type: "REL", value: 1.0 });
            bal.reset(100, 1000);
...
describe("BaseSpecialIndicator", () => {
    ...
    test("getConfig returns full config", () => {
        const sl = createStopLoss("LONG", { type: "REL", value: 0.02 });
        const config = sl.getConfig();

        expect(config.direction).toBe("LONG");
        expect(config.stopLoss).toEqual({ type: "REL", value: 0.02 });
        expect(config.trailing).toBe(false);  // Defaults to false when not specified
    });
});
