...
} from "../../../src/simulation/stages/indicator-calculation.ts";
...
function createMockSignalCache(entries: [string, boolean[]][]): SignalCache {
...
function createMockIndicatorResult(overrides: Partial<IndicatorCalculationResult> = {}): IndicatorCalculationResult {
    ...
    return {
        signalCache,
        warmupCandles: 10,
        indicatorConfigs: [{ type: "RSI", params: {} } as any, { type: "EMA", params: {} } as any],
...
describe("createIndicatorInputFromDataResult", () => {
    ...
    it("extracts algo params from data result", () => {
        ...
        expect(input.algoParams).toEqual(dataResult.validatedInput.algoConfig.params);
    });
...
describe("getSignalAtBar", () => {
    it("returns signal at valid index", () => {
        const signalCache = createMockSignalCache([["test", [true, false, true, false]]]);
    ...
    it("returns undefined for unknown key", () => {
        const signalCache = createMockSignalCache([["test", [true, false]]]);
    ...
    it("handles empty signal array", () => {
        const signalCache = createMockSignalCache([["test", []]]);

        expect(getSignalAtBar(signalCache, "test", 0)).toBeUndefined();
    });

    it("handles single element array", () => {
        const signalCache = createMockSignalCache([["test", [true]]]);
    ...
    it("handles boundary index (last element)", () => {
        const signalCache = createMockSignalCache([["test", [true, false, true]]]);
