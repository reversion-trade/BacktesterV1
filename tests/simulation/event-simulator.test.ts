...
import { runEventDrivenSimulation, type EventSimulatorConfig } from "../../src/simulation/event-simulator.ts";
...
describe("Event Simulator - End of Backtest", () => {
    ...
    test("leaves position open when not configured to close", () => {
        ...
        expect(result.finalState).toBe("POSITION");
    });
});
...
describe("Event Simulator - Statistics", () => {
    ...
    test("state transitions are recorded", () => {
        ...
        // Should have: CASH → POSITION, POSITION → CASH
        expect(result.stateTransitions.length).toBe(2);
        expect(result.stateTransitions[0]!.fromState).toBe("CASH");
        expect(result.stateTransitions[0]!.toState).toBe("POSITION");
        expect(result.stateTransitions[1]!.fromState).toBe("POSITION");
